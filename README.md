...
Every SVG in `assets/figures/` comes out of a script in `scripts/figures/`, one
per article, run by hand:

```
node scripts/figures/orden-de-merito.mjs
```

The build does not run them. It copies `assets/` as it finds it, so the SVGs are
committed and a figure only changes when its script is re-run deliberately.

The rule that makes this worth the trouble is at the bottom of each script: the
numbers the figure prints are **computed from the example data and then checked
against what the article says**, and a mismatch exits non-zero. That is what
stops a caption and a paragraph from drifting apart after an edit. If a check
fails, one of the two is wrong and the script will not tell you which.

Colours are the `style.css` tokens, hardcoded, because the files are referenced
with `<img>` and cannot inherit the page's custom properties.

Any value that is illustrative has to say so in the figcaption, in bold. See the
authoring spec, section 5.

Two kinds of figure live outside `scripts/figures/`, and both are deliberate:

- **matplotlib PNGs** for dense data plots, generated by `figuras.py` inside the
  relevant project. Hand-rolling SVG for a 526-point series would be a lot of
  work for a worse result.
- **animated GIFs**, also from matplotlib, for the handful of concepts where
  movement teaches something a still cannot: the central limit theorem as the
  sample size grows, the coverage of a confidence interval over repeated
  samples, and how forecasts separate along the horizon. The bar for animating
  is high — a still can be studied for as long as the reader wants, a GIF
  cannot — so most figures stay static.
